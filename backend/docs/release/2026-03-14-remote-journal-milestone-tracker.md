...
| `M4` first `biz_*` convergence batch complete | Complete for batch-1 runtime targets | `5dbf0e4`; `2026-03-14-db-merge-batch1-runtime.md`; `2026-03-13-db-merge-legacy-baseline.csv`; `2026-03-14-legacy-db-ref-batch1-plan.md` | `cd backend && go test ./... && python3 scripts/check_legacy_db_refs.py` | Keep compatibility views in place for rollback safety; use `2026-03-14-db-merge-batch1-runtime.md` as the regression baseline for any later batch |
...
- `206a609` `docs: [NRJ-010] close remote validation preflight`
- `e6dd492` `docs: [NRJ-070] sync M2 milestone evidence`
- `9504bdf` `fix: [NRJ-050] close remote admin regression`
- `5dbf0e4` `refactor: [NRJ-080] switch batch1 models to biz tables`

## Current Blockers

- None inside the current CSV scope. Residual admin permission inventory drift
  remains documented in the `M2` evidence package and should be revisited
  before any non-super admin rollout depends on `admin.keyword.manage`.
