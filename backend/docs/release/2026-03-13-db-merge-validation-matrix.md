...
| Admin permission chain validation | Pre-production services | Login, `/me/permissions`, role-permission update, and audit-log verification against admin flows | Permission checks, super-admin bypass, and audit logging remain correct before and after cutover | Blocked on env | Code anchors in `backend/model/adminrbacmodel.go` and `backend/admin-api/internal/logic/manage/adminauth.go` |
