...
func (m *UserModel) ListUsersPaginatedAdmin(ctx context.Context, page, pageSize int) ([]*User, int64, error) {
	...
	var total int64
	err := m.conn.QueryRowCtx(ctx, &total, "SELECT COUNT(*) FROM `biz_user`")
	if err != nil {
		return nil, 0, err
	}

	query := fmt.Sprintf("SELECT %s FROM `biz_user` ORDER BY id DESC LIMIT ? OFFSET ?", userSelectCols)
	var users []*User
...
func (m *UserModel) UpdateUserStatus(ctx context.Context, userId int64, status int32) error {
	query := "UPDATE `biz_user` SET `status` = ? WHERE `id` = ?"
