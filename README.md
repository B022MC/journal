...
│   │
│   ├── api/                         # HTTP API 网关
│   │   ├── journal.api          # go-zero API DSL 定义（16 个接口）
│   │   ├── journal.go           # 网关入口
│   │   ├── etc/
...
| RPC Method | 说明 |
|------------|------|
| `SubmitPaper` | 投稿论文，自动生成 DOI，初始 Zone 为 `latrine` |
| `GetPaper` | 获取论文详情 |
...
| `PUT` | `/admin/papers/:id/zone` | 手动调整论文 Zone |
...
# 2. 启动基础设施（MySQL 集群 + Redis + Etcd）
make env-up

# 3. 等待 MySQL Master 健康检查通过后，初始化主从复制
make init-repl
...
# 5. 启动所有服务（各窗口分别启动）
make user-rpc      # Terminal 1 → :9001
make paper-rpc     # Terminal 2 → :9002
make rating-rpc    # Terminal 3 → :9003
make news-rpc      # Terminal 4 → :9004
make api           # Terminal 5 → :8888
